...
===============================================================================
  ...
  /home-bank/notre-marque-employeur/nos-offres-demploi/
  Parsing best-effort des liens/titres d'annonces ; retour vide honnête si
  la page est vide ou dynamique (pas de fausses offres).
===============================================================================
"""
...
_NAV_WORDS = ("accueil", "connexion", "inscription", "contact", "à propos", "a propos",
              "mentions", "politique", "nos offres d'emploi", "se connecter", "retour")


class NsiaScraper(BaseScraper):
    name = "nsia"
    base_url = "https://www.nsiabanque.ci"
    listing_url = "https://www.nsiabanque.ci/home-bank/notre-marque-employeur/nos-offres-demploi/"
    ...
    def scrape(self, max_offers: int = 10) -> List[Job]:
        ...
        if soup is None:
            self.logger.warning("NSIA Banque CI injoignable. Aucune offre récupérée.")
            return jobs

        # Best-effort : blocs d'annonces ou liens explicites.
        candidates: List[tuple[str, str]] = []
        for sel in (".job-item", ".job-offer", ".offre", "article", ".card"):
        ...
        if not candidates:
            for a in soup.select('a[href*="offre"], a[href*="emploi"], a[href*="poste"]'):
        ...
        for title, href in candidates[:max_offers]:
            try:
                ...
                self.logger.debug(f"Erreur sur une offre NSIA : {exc}")

        if not jobs:
            self.logger.warning(
                "NSIA Banque CI : aucune offre détectée sur la page carrières. "
